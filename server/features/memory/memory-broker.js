...
import { privateFolder } from "./memory-validation.js";
import { memoryResponse } from "./memory-protocol.js";

export class MemoryBroker {
  constructor(memory) {
    this.memory = memory;
    const hash = createHash("sha256").update(memory.root).digest("hex").slice(0, 20);
    this.socketPath = `/tmp/agentpier-memory-${process.getuid?.() ?? "user"}-${hash}/mcp.sock`;
  ...
  async start() {
    ...
    try {
      ...
      if (!previous.isSocket() || (process.getuid && previous.uid !== process.getuid()))
        throw Error("Unsafe Memory socket.");
    } catch (error) {
    ...
    this.server.setTimeout(10000, (socket) => socket.destroy());
