...
export const appDocumentPath =
  /^\/(?:accounts|repositories|files|settings(?:\/(?:notifications|diagnostics|backups|updates|audit|mcp|ssh))?|memory(?:\/[^/]+)?|pipelines(?:\/(?:runs|definitions|profiles|verification)(?:\/[^/]+)?)?|agentbus(?:\/messages(?:\/[^/]+)?)?|(?:extensions|plugins)(?:\/[^/]+)?|sessions\/[^/]+(?:\/(?:chat|reader|terminal|files))?)?\/?$/;
