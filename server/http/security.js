...
export const appDocumentPath =
  /^\/(?:accounts|repositories|files|artifacts(?:\/(?:view\/)?[A-Za-z0-9_-]+)?|settings(?:\/(?:notifications|diagnostics|backups|updates|audit|mcp|remote|ssh|github))?|projects(?:\/[^/]+)?|memory(?:\/[^/]+)?|pipelines(?:\/(?:runs|definitions|profiles|verification)(?:\/[^/]+)?)?|agentbus(?:\/messages(?:\/[^/]+)?)?|(?:extensions|plugins)(?:\/[^/]+)?|sessions\/[^/]+(?:\/(?:chat|reader|terminal|files))?)?\/?$/;
...
export function securityHeaders(req, res, next) {
  ...
  if (/^\/artifacts\/view\/[A-Za-z0-9-]+\/?$/.test(req.path)) {
    res.setHeader(
      "Content-Security-Policy",
      "default-src 'self'; script-src 'self' data: 'unsafe-inline'; style-src 'self' data: 'unsafe-inline'; connect-src 'self'; img-src 'self' data:; font-src 'self' data:; frame-src 'self'; object-src 'none'; base-uri 'none'; frame-ancestors 'none'",
    );
  }
  next();
}
