...
function entry(name, type, folder = "/home/test", extra = {}) {
  return {
...
async function explorerFixture(
  ...
  await page.route("**/api/files/**", async (route) => {
    ...
    const selectedPath = url.searchParams.get("path") || context.home;
    if (url.pathname.endsWith("/metadata")) {
      const name = selectedPath.split("/").at(-1);
      return route.fulfill({
        json: entry(name, name === "linked-docs" ? "symlink" : "file", "/home/test"),
      });
    }
    ...
    if (url.pathname.endsWith("/entries")) {
      const entries =
        selectedPath === "/home/test/docs" || selectedPath === "/home/test/linked-docs"
          ? [entry("guide.txt", "file", selectedPath)]
          : [
              entry("docs", "directory"),
              entry("linked-docs", "symlink"),
      ...
      return route.fulfill({
        json: {
          path: selectedPath,
          parent:
            selectedPath === "/" ? null : selectedPath.replace(/\/[^/]+$/, "") || "/",
          entries,
