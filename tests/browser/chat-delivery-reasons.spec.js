...
test("unconfirmed Claude image chips are explained in English as unsent", async ({
  page,
}) => {
  await fixture(page, {
    status: "uncertain",
    reason: "CHAT_IMAGES_UNCONFIRMED",
    error: "Die Nachricht wurde nicht abgeschickt.",
  });
  await input(page).fill("Describe the attached screenshot");
  await page.getByRole("button", { name: "Send", exact: true }).click();
  await expect(
    page.getByText(
      /message was not submitted: Claude's input field does not show all attached images/,
    ),
  ).toBeVisible();
  await expect(page.getByText(/nicht abgeschickt/)).toHaveCount(0);
});
