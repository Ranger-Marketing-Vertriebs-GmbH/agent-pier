...
test("waiting for chips polls until the exact count is visible", async () => {
  ...
  assert.equal(await waitForClaudeImages(ready.manager, { id: "synthetic" }, 2), true);
  ...
  assert.equal(
    await waitForClaudeImages(more.manager, { id: "synthetic" }, 4, { timeoutMs: 100 }),
    false,
  );
  // Without a readable baseline, at least the new chips must show.
  const lower = waiting([first.chips3Only60x8]);
  assert.equal(
    await waitForClaudeImages(lower.manager, { id: "synthetic" }, 2, { exact: false }),
    true,
  );
  const stuck = waiting([screen("[Image #1]")]);
  assert.equal(
    await waitForClaudeImages(stuck.manager, { id: "synthetic" }, 2, { timeoutMs: 0 }),
    false,
  );
});

test("scrolled chips keep waiting, then report them as unconfirmed", async () => {
  const scrolled = waiting([screens.images3Scrolled60x11]);
  assert.equal(
    await waitForClaudeImages(scrolled.manager, { id: "synthetic" }, 3, {
      timeoutMs: 150,
    }),
    false,
  );
...
test("a chips-only prompt is recognized also when the chips wrap", () => {
  const chipsOnly = (name, n) => claudeChipsOnly(first[name].raw, first[name].pane, n);
  assert.equal(chipsOnly("chips3Only60x8", 3), true);
  assert.equal(chipsOnly("chips3Only40x10", 3), true);
  assert.equal(chipsOnly("chips3Wrapped30x10", 3), true);
  assert.equal(chipsOnly("chips3Wrapped30x10", 2), false);
  // Text after the chips, or a draft scrolled past them, is not chips only.
  for (const name of ["chips3Short60x8", "chips3Long60x8", "chips3Long40x10"])
    assert.equal(chipsOnly(name, 3), false, name);
  assert.equal(chipsOnly("chipsRenumbered120x35", 2), false);
  assert.equal(claudeChipsOnly("invalid", {}, 0), false);
});

test("a dialog hiding the prompt ends the chip wait at once", async () => {
  const { permission } = await fixture("claude-2.1.280-screens.json");
  const dialog = waiting([permission]);
  const started = performance.now();
  assert.equal(
    await waitForClaudeImages(dialog.manager, { id: "synthetic" }, 1),
    "dialog",
  );
