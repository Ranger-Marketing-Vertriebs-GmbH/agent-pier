...
for (const explicitVersion of [false, true])
  for (const version of ["1.0.0", "2.0.0", "3.0.0-beta.1"]) {
    test(`stable tap retains version 2.0.0 for candidate ${version} (explicit version: ${explicitVersion})`, async (t) => {
      const { updateHomebrewTap } = await import("../../scripts/homebrew-tap-update.mjs");
      const { renderInstallerFormula } =
        await import("../../scripts/homebrew-formula.mjs");
      ...
      const rendered = renderInstallerFormula({
        version: "2.0.0",
        file: "agentpier-installer-2.0.0.tar.gz",
        sha256: "a".repeat(64),
      });
      const formula = explicitVersion
        ? rendered.replace("  sha256", '  version "2.0.0"\n  sha256')
        : rendered;
