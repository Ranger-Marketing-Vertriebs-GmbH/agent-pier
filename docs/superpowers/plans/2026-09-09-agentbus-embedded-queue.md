...
- [ ] **Step 1: Write failing tests** for duplicate enqueue, two concurrent owners claiming disjoint rows, lease expiry, owner-scoped ack, and message-size validation.
- [ ] **Step 2: Run `node --test tests/unit/agentbus-queue.test.js` and confirm the new imports/functions fail because the store does not exist.**
- [ ] **Step 3: Implement the schema, WAL setup, transactions, lease recovery, and private file creation in `queue.js`.**
- [ ] **Step 4: Run the focused test and confirm all queue-store cases pass.**
- [ ] **Step 5: Commit `feat: add durable AgentBus SQLite queue`.**
...
- [ ] **Step 1: Add failing integration tests** proving a second reader cannot consume the same claim, a crashed claim is retried after lease expiry, and duplicate `peer_send` does not create a second row.
- [ ] **Step 2: Run the focused integration tests and confirm they fail against file-based inbox behavior.**
- [ ] **Step 3: Replace file enqueue/read/count operations with queue-store calls and acknowledge only after `formatMessages` succeeds.**
- [ ] **Step 4: Keep the existing nudge return contract while making its count a non-destructive queue summary.**
- [ ] **Step 5: Run all AgentBus integration tests and commit `fix: claim and acknowledge AgentBus messages`.**
...
- [ ] **Step 1: Add failing migration/history tests** for pending files, done files, malformed files, pagination, and repeated initialization.
- [ ] **Step 2: Run the focused tests and confirm the new SQLite history is empty or missing.**
- [ ] **Step 3: Implement idempotent legacy import and replace filesystem history scanning with bounded SQL queries.**
- [ ] **Step 4: Run AgentBus integration and browser fixture tests.**
- [ ] **Step 5: Commit `feat: migrate AgentBus history to durable queue`.**
...
- [ ] **Step 1: Document queue states, lease recovery, migration location, and the fact that nudges are not delivery acknowledgements.**
- [ ] **Step 2: Run `npm run lint`, `npm run format:check`, `npm run check:structure`, `npm run build`, and `npm test`.**
- [ ] **Step 3: Inspect the diff and working tree, then commit `docs: document AgentBus queue recovery`.**
