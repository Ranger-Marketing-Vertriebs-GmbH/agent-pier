...
benannten Übersprüngen und ohne Fehler. Auch Chromium/WebKit, Secrets und CodeQL
bestehen: alle 17 CI-Prüfungen sind erfolgreich.
...
- [x] Accept reviewed source `0e3a4d7` in Linux/macOS Node 22/24 backend CI and all four actual release-package targets; publication remains skipped. All 17 CI checks, including Chromium/WebKit, Secrets and CodeQL, passed on that exact head.
...
- [x] Accept reviewed source `82bb9f7` in all Linux/macOS Node 22/24 backend CI jobs. All 17 checks, including Chromium/WebKit, passed on that exact source head; final whole-branch acceptance still requires current checks.
...
- [x] Test contiguous Shift selection, toggles, stale selection after reload, failed-cut retention and modal dismissal. Browser tests use realistic job fixtures with per-entry success and failure.
...
- [x] Build and run `npx playwright test tests/browser/file-explorer-actions.spec.js tests/browser/file-explorer-trash.spec.js`; confirm controls are initially absent.
- [x] Add visible action menus, context menus and selection checkboxes, create/rename dialogs, copy/cut/paste and path-copy. Scope clipboard operations again on the server. After partial cut, remove only successful source IDs from the clipboard.
...
- [x] Make conflict choices type-aware, with an explicit apply-to-remaining checkbox. Implement default trash, restore, empty and permanent-delete confirmations bound to a frozen selection; show disk usage and reason `deleted`/`replaced`. Render interrupted adoption/recovery items with retained locations and recovery actions, never hide them as ordinary failures.
- [x] Verify cancellation, errors and folder refresh; run both browser engines and `node --test tests/unit/file-selection.test.js tests/unit/i18n-catalogs.test.js`.
- [x] Commit: `git commit -m "feat: add file actions and trash controls"`.
...
Ruling: T10 journaled public-source rename — Add the internal FilePublisher.rename(scope,sourcePath,targetPath,{jobId,sourceRevision,expectedRevision,signal}) composition using the existing native stage/publication machinery. Bind it to a real trusted job/scope, owned source and target parents, selected source identity/type/revision and journaled original provenance before moving the same inode into a private stage. Keep trash-only stage.adoptEntry authority narrow; no raw/public adoption capability. Revalidate under short publisher-owned source/target leases, sync retained parents, then use existing no-replace/exchange publication and return displaced recoveryId for trash.adoptDisplaced. A case-only intermediate may expect absence only after proving the spellings denote the same directory entry and vacating it; same inode alone is insufficient when distinct hardlinks exist. On pre-publication failure, return the matching source to its original spelling only with matching original parent and native no-replace into proven absence; otherwise preserve a registered, discoverable source-recovery obligation and all bytes. Never discard an adopted public source as an empty/abandoned stage or overwrite an unrelated new source/target. Accepted composed operations must own/drain every internal phase through cancellation and close, without holding outer leases during long work. Document exact recovery disposition and test failures around adoption, publication, retry and original-path replacement — Task9 stage adoption deliberately grants authority only to private trash restores, while Task10 requires same-inode rename and replacement through the existing journaled publisher rather than a strict new-inode copy or a second mutation engine — cost if wrong: source-adoption journal/recovery and lifetime contracts need rework, and interrupted renames may temporarily retain the original under a private registered location until safe recovery.

Ruling: T10 interrupted-rename recovery projection — Add internal FileTrash.retainRenameSource(scope,recoveryId) for a still-unresolved publication carrying verified public renameSource provenance whose registered stage actually contains that source inode. Register the existing retained stage as trash without another move/copy, using the original selected source path, reason interrupted_rename and paired localized FILE_RENAME_RECOVERY messaging. Reuse normal current-scope filtering, private location checks and restore/purge machinery; expose recoverable only after bounded identity/manifest verification, otherwise a visible pending state. Direct failure handling and startup replay may invoke it idempotently; retries never reset existing location, completion or recovery evidence. Do not adopt a published target or a displaced destination as the original source, and do not unpin/resolve any publication until all recorded source, displaced-data and container obligations are durable. If registration itself fails, retain the previously journaled source obligation and bytes for later replay; avoid promising immediate restore for a pending entry. No HTTP path/descriptor capability or private staging path is exposed — the journaled rename composition can preserve its source after an unsafe rollback, but the owner also needs a truthful, reachable recovery entry; deleted would misdescribe an interrupted rename — cost if wrong: the private source-adoption projection and downstream reason/status UI contracts need rework, and uncertain sources may remain pending until their location and manifest can be proven.

Ruling: T10 unverified-source pending visibility — Permit retainRenameSource/startup reconciliation to project a metadata-only pending trash entry from a validated server-owned rename publication and its original job/scope/source provenance when a relocated or inaccessible ancestor prevents proving the registered payload location. Preserve the journaled locations and completion evidence; do not claim current bytes exist or that a guessed pathname identifies the source. This pending projection has revision null and grants no restore/purge/mutation capability; all existing global/current-project filtering and private-path redaction remain. Upgrade only after the same descriptor/parent/inode/manifest proof required for normal recoverable entries, and keep retries idempotent without repurposing an existing record. Pending entries and publication obligations remain pinned with no expiry; unproven paths are never opened for mutation or swept. Distinguish this metadata-only visibility from Ruling26s proof required to expose a usable source — an external ancestor relocation can make the recorded source inaccessible, but omitting it from recovery views would violate the discoverability requirement and make the rename-recovery message misleading — cost if wrong: pending projection and reconciliation fields need rework, and the owner may see an entry that remains unavailable until the original location can be proven.

Ruling: T11 cross-parent same-inode move — Extend the existing internal FilePublisher.rename composition to source and destination parents in the same trusted scope, retaining the public rename handlers same-parent contract. Validate canonical identity, own-descendant rejection and both current parents before adoption/publication; case-only name evidence applies only within the same directory, and cross-parent same-inode hardlinks never imply a vacated target. Keep separate retained source, private-stage and target-parent authority throughout rollback/replay: inspect and remove the registered stage container through its actual verified target parent, including the already-removed-container path, and resolve only after every recorded obligation is durable. A proven native EXDEV before source adoption may select strict copyVerified only after confirming the original source remains unchanged and the attempted stage is safely resolved; expose that distinction only to trusted internal callers, without raw diagnostic HTTP fields or fallback from unrelated failures. Preserve all source/displaced recovery visibility, bounded short leases and accepted-operation drainage — Task11 requires same-filesystem cross-folder moves through native rename, but Task10 explicitly rejected different parents and its rollback assumed the stage lived under the source parent; this is a necessary narrow extension of the existing engine — cost if wrong: parent-specific rollback/replay and EXDEV classification need rework, and uncertain attempted moves must remain retained rather than proceeding with a second copy.

Ruling: T11 durable transfer completion — Add a private transferId association between a real job, bounded per-entry transfer intent and the existing stage/publication document. Persist trusted source identity, intended scoped output and operation kind before stage/adoption; reject missing or mismatched associations rather than treating an arbitrary UUID as authority. Use one idempotent FileStore completion transaction for exact job/transfer rows, counters and completion evidence under the final short publication barrier, before container cleanup or publication resolution. Startup invokes that same checkpoint only after existing target/inode/hash and parent proof, covering publication-syscall success before the live checkpoint; hashes remain outside the barrier. Distinguish outputPublished from sourceRemoved: copy may complete at durable output, a native move requires proven completed source relocation, and EXDEV move remains incomplete until matching source removal is durable. Interrupted or partial move results must truthfully retain both the output evidence and unfinished source; retries neither repeat completed movement nor double-count entries/bytes. Keep internal source identities, paths and transfer associations out of HTTP projections unless already part of the bounded public result contract — publication currently resolves before handler result storage, leaving a restart window in Task11s required per-entry completion accounting; an in-memory callback alone cannot replay that window — cost if wrong: private entry/publication schema and counter reconciliation require rework, and ambiguous completed operations must remain interrupted rather than be retried automatically.

Ruling: T11 unpublished copy-stage cleanup — Extend the existing verified-transfer checkpoints with registered root identity and bounded child-creation intent before each native create, followed by awaited created-identity and progress evidence. Permit finite reverse cleanup only of proven entries in the same still-private unpublished registered stage, using retained parent authority and immediate identity/type plus available mutation evidence; then invoke the existing publisher discard/finalization path. Reuse native/tree ownership and keep long traversal/hash work outside global leases. Creation intent without matching identity proof is uncertainty, never permission to remove a guessed name; a crash or checkpoint failure between create and identity recording keeps the unresolved stage pinned. Preserve observed replacement/unknown content, every source, already-published outputs and all journal markers; do not sweep by names or extend cleanup to adopted public/trash sources. Carry the existing finite native-writer check/action boundary and durably record completed cleanup before resolving — Task11 must cancel and clean its own failed staged copies while copyVerified can leave populated stages that ordinary empty-stage discard cannot remove; current awaited targetIdentity checkpoints provide the narrow basis, but pre-create uncertainty must remain explicit — cost if wrong: transfer creation/removal checkpoints and stage replay need rework, and uncertain private work may remain retained until its identity and disposition can be proven.

Ruling: T11 existing-directory merge metadata — Explicit directory merge combines child entries while retaining the existing destination directory inode, ownership, mode, ACLs and extended attributes; do not overwrite that pre-existing directory with source-directory metadata or reapply source timestamps. Normal directory timestamp changes caused by inserting/removing children remain possible. Newly created/copied directories still receive source metadata after their children, and each transferred child retains its ordinary copy or strict-move preservation requirements. Record a bounded paired localized FILE_MERGE_METADATA_RETAINED informational issue for an explicit merge and carry the same explanation into the later conflict dialog, so the existing-root exception is visible rather than silently presented as full source-directory metadata preservation. This applies to explicit copy and move merges; the source directory container is not represented as a newly copied destination inode — applying source ACL/ownership to an already-populated destination would also change access to unrelated existing children, while the specification distinguishes explicit merge from replacement and requires deviations to be reported — cost if wrong: merge metadata policy and its UI explanation need rework, and source-root attributes are intentionally not reproduced on the existing merged directory.

Ruling: T11 clipboard source preconditions — Copy and move additionally accept options.revisions as an optional exact map from every original requested source path to its actual string e1 revision, with no unknown option keys, missing/extra source keys, alternate revision schemes or coercion. Empty options retains server-observed planning for callers without preconditions; when revisions are supplied, compare all original selected entries before de-duplication can erase a selected child and before any namespace mutation. A stale supplied source revision fails with FILE_CONFLICT_CHANGED without copying/removing that requested selection; continue existing fresh manifest/current-context revalidation after planning and conflict waits. Task12 always sends its stored scoped path/revision references when executing copy/cut/paste. Request-id binding includes those preconditions. Preserve the existing per-entry partial-result rules for failures after admitted planning — the final Task11 report rejects every option, but Task12s approved clipboard interface stores revisions and its move request explicitly sends options.revisions; silently dropping those values would change selection semantics and break the planned consumer — cost if wrong: copy/move admission and clipboard compatibility need rework, and stale selections may require refreshing before retry.

Ruling: T12 typed whole-entry restore conflicts — Extend the existing restore handler with Keep both and bounded source/destination type fields. Matching non-directory types offer replace, skip, keep_both and cancel; directory or mixed-type pairs offer keep_both, skip and cancel. Restore retains its whole-trash-entry private adoption contract; explicit directory merging remains a copy/move action after separate restoration, and no unusable restore Merge or implicit directory replacement is offered. Bind a decision to captured trash t1, current ownership and destination e1/canonical identity before HTTP acceptance; recheck the trusted source revision, current scope and type at actual adoption/publication, including direct expectedRevision callers. Reserve an alternate through existing name generation and atomic no-replace publication, never retry after adopted/published uncertainty or expose private paths — Task12 requires Keep both for restore, but the accepted Task9 handler currently exposes only replace/skip/cancel without types; a narrow integration preserves its journal instead of inventing a second restore engine — cost if wrong: restore conflict validation/UI need rework, and merging a restored directory requires a separate explicit copy/move step.

Ruling: T12 frozen bounded purge batches — Load every trash page before Empty confirmation and freeze the recoverable IDs, t1 revisions and public selection metadata, displaying nonrecoverable entries separately. Use sequential immutable purge requests whose complete UTF-8 JSON envelopes fit the existing 64 KiB cap. Each submitted batch retains the existing all-member preflight before its first removal and each-entry revalidation; do not promise a single cross-batch preflight or atomic transaction. Stop later submissions after failure, cancellation or owner replacement, never add entries arriving after consent, and show proven per-ID completed, failed/uncertain and unsubmitted results instead of inferring success from counters. Known usage covers all loaded pages with explicit unknown sizes. Necessary stable result projections belong here under F1; no new durable confirmation service is required — Task9 explicitly assigns frozen paginated batching to Task12, and its whole-submitted-selection preflight plus immutable consent preserves the approved non-atomic multi-operation behavior within the JSON bound — cost if wrong: batch orchestration and outcome UI need rework, and earlier confirmed batches may finish before a later batch fails validation.

Ruling: T13 durable group coordination — Keep bounded group manifests in parent job entries, with private manifest state and canonical batch receipts committed atomically with aggregate limits, implicit-directory accounting and child-entry claims. Freeze and validate the manifest before visible directory creation. Use finite internal upload_group aggregation passes in the existing dispatcher with non-transfer policy, bounded joins, a durable generation and coalesced wake; ordinary handlers retain their completion semantics. Dormant unattempted files remain queued without a worker or transfer slot, never successful because the browser is quiet. Map actual create_directory results: reuse a revalidated matching directory, propagate an alternate parent path to descendants and skip or fail blocked subtrees explicitly. A fresh request for an unfinished entry may wake eligible committed group bookkeeping only after the prior attempt drains and its publication obligations are resolved; old request replay returns the old child without waking, and completed/skipped entries never reopen. Cancellation intent survives late callbacks; startup performs metadata interruption only, never recreates directories or replays byte reservations. Wake on admission/disposition changes, not every byte report — current runReserved forces every group to occupy a transfer slot and lacks manifest/child atomicity, which can deadlock three groups waiting for their own children and falsely complete dormant groups — cost if wrong: group lifecycle and transactional admission need rework, with interrupted unfinished entries requiring explicit retry.

Ruling: T13 logical upload reservation — Interpret create as a durable private upload/target reservation with immutable scoped request metadata, without allocating physical stage handles for dormant uploads. Allocate the actual existing publisher stage only after receive admission has a transfer slot and a verified ready parent. No final target is created by reservation, no raw descriptor or synthetic stage capability is exposed, and reservation ownership still precedes every actual stage operation — the current stage retains three handles and release permanently revokes it, so eager allocation bypasses the three-transfer resource bound; a suspend/reopen mechanism is unnecessary — cost if wrong: reservation/stage timing needs rework and filesystem allocation failures can first appear when receiving starts.

Ruling: T13 cumulative upload byte accounting — Charge actual admitted byte chunks to the child attempt and durable group attempted-byte budget in one transaction before forwarding them to owned writes; await bounded progress persistence and final flushing. Enforce declared exact final length independently of Content-Length, the per-file cap and the aggregate cap. Failed attempts do not refund processed-byte budget; a completed response replay is not charged or written again. Explicit retries use new child requests and preserve completed entries. Public counters describe the current logical entry results and must not claim transfer completion merely because all transport bytes arrived; retain the cumulative retry charge privately — independent child reports otherwise race the group cap and discarded progress promises defeat backpressure — cost if wrong: byte/progress accounting and retry UX need revision, and exhausting the cumulative group budget requires a new explicitly selected operation.

Ruling: T13 journaled upload completion — Extend the existing publication association and completion dispatch with a distinct private upload association bound to the actual upload job, scope, immutable declaration and optional group entry before stage creation. Do not forge copy/move source-inode manifests. Atomically checkpoint proven upload publication, child/group result evidence and the completion marker before publication cleanup, using the existing recovery proof and transaction machinery. Distinguish published-byte evidence from full operation success while displaced-data adoption or cleanup remains unresolved. Existing startup reconciliation may record proven publication but never restart a receiver. A duplicate active request has one writer; replay of a durably completed upload returns its original result without publishing again, and uncertain published/adoption work cannot be silently replaced by a retry. Verify streamed bytes against the completed owned payload before treating upload content as proven — Task13 requires lost-response/crash idempotency, while the current completion association accepts only copy/move and updates one job — cost if wrong: private publication/group checkpointing needs rework and uncertain uploads must remain visible instead of being repeated.

Ruling: T13 shared direct-download admission — Count direct downloads against the same three owner transfer slots as uploads and file transfers. Add private ephemeral read-only transfer items to the existing FileJobs pending queue, active ownership, worker set and transfer counter; do not create a second semaphore or public durable download job. Refresh trusted scope after admission, open descriptors only inside the admitted callback, remove aborted queued items, and drain active streams before releasing their slot or closing services. Headless/read-only project downloads remain allowed; no mutation permission is gained — direct downloads currently have no scheduler admission and the existing reserved path is writable-upload-only — cost if wrong: shared admission and close/abort handling need rework and downloads may wait behind other active transfers.

Ruling: T13 versioned portable upload-name rejection — For upload-group planning, reject sibling spelling collisions using a persisted policy version and component key NFD(full default Unicode15.1 case fold(NFD(component))), with pinned C/F mappings, excluding Turkic mappings. Include implicit parent components in the trie, share identical ancestor spellings, reject malformed UTF-16 and preserve original accepted names. Ship the pinned mapping data and its license with the application; this is a documented product rejection rule, not proof of every target filesystem's name equivalence. Continue actual target identity/conflict revalidation and atomic no-replace; never silently coalesce distinct planned parents when actual aliases are observed. Do not claim a scratch directory elsewhere proves the final parent's policy or that this rule satisfies Task16's stronger actual-target requirement; carry that separate proof decision to Task16/22. The rule may reject otherwise distinct names on case-sensitive filesystems and cannot certify filesystem-specific equivalences — the current native ABI exposes no comparison-policy capability, and ext4 permits different case behavior per directory even on one filesystem, so a universal scratch-probe claim would be false — cost if wrong: name validation and the later actual-target acceptance policy need revision, and some valid local sibling names require an explicit renamed/new upload selection.

Ruling: T16 actual-parent filename attestation — Preserve ordinary extraction layout and explicit merges by certifying each effective sibling set before any result publication. Use exact-name exclusive witnesses in registered private probes, and attest each existing final parent/probe pair through worker-owned descriptors on the same live native APFS volume or unencrypted native ext4 mount/filesystem. APFS requires matching filesystem/device identity and valid volume case capability evidence; ext4 requires descriptor-bound mount/type identity, matching observed CASEFOLD flags and no encryption. Verify actual inheritance instead of assuming it; Linux statx mount identity/procfs/GETFLAGS availability is a capability requirement for this operation, not a raised global OS floor. The live filesystem compares original names; neither a pinned userspace fold nor a scratch directory without proven policy equivalence constitutes the required evidence. An exactly retained tested output-subtree inode can retain its own sibling proof, but must not silently add a wrapper, replace an existing merge parent or certify multiple insertions into an unproved existing parent. Resolve effective parent/name choices before certification, include implicit parents and file-parent collisions, and invalidate affected proof when a choice, scope, parent identity or policy changes. Unknown filesystem/query/permission/policy evidence or known aliases refuse before first output; do not fall back to portable-only acceptance. Revalidate under the existing short publication seam; observed later changes after earlier outputs stop affected work with explicit partial results, retaining the acknowledged same-user check-to-action residual. Keep registered creation intents, bounded O(entries) witnesses/manifests, <=50000 planned entries/128 depth, finite proven cleanup and uncertain retention in the existing engine; no raw descriptors, target placeholders, global lease during probing or new cleanup runner. Bring the narrow fixed native capability and actual APFS/ext4, four-architecture release/install/update validation into Task16/22 under F1 — existing-directory merges preserve their inodes, so testing a different unproven parent cannot satisfy the approved actual-target prepublication alias requirement; primary APFS volume and ext4 filesystem/per-directory contracts provide a bounded implementation — cost if wrong: native proof, extraction planning and cleanup need rework; extraction requiring unproved existing parents is unavailable on other filesystems, encrypted ext4 or hosts lacking the necessary descriptor queries, while normal Explorer operations remain available.

Ruling: T14 bounded upload recovery discovery — Bring the minimal paged history and explicit source-reselection controls needed for upload recovery into Task14 using the existing FileJobClient/useFileJobs queue and observer. Load history only in bounded pages on explicit navigation, retain/adopt the selected scoped upload group, and fetch its complete bounded manifest from an explicit first page through opaque cursors before matching RAM File references to server-confirmed immutable declarations and IDs. Show a clear missing-source notice after reload; never persist File/Blob/content references or start bytes until the user explicitly reselects matching unfinished entries. Keep completed/skipped entries closed, preserve published/uncertain states and backend retry eligibility, and keep each original destination/remap bound to its group. A group beyond the initial200 history rows remains reachable through explicit paging; do not silently scan all history or promise automatic local-file recovery. Task17 retains full operation/artifact/history presentation beyond this necessary upload entry point. Ordinary helper extensions carry the opened scope/destination/effective limits and retained attempt explicitly, mount transfer ownership above path-keyed FileActions, and reuse scoped job-envelope adoption, queued explicit-page reads and shared XHR error/auth handling under F1. Provide a labelled external drop area for the displayed destination plus native file/folder inputs; internal gestures remain isolated, with directory-row external targeting outside this initial upload surface — the approved reload/reselection behavior would otherwise lose untracked groups outside the first page, while the brief's helper lacks its write context and the current observer cannot adopt upload envelopes or return an explicit page-one scan — cost if wrong: browser ownership, history/reselection and client adapter integration need rework; reload recovery requires manual paging and local source reselection rather than automatic discovery or persistent access.

Ruling: T15 operation-dependent archive authority — Determine archive read permission from its validated immutable output mode. Download archives may read in a read-only/headless project, including a readable selected project root and version-bound omission consent; file-output archives retain ordinary mutation authority. Apply the same private operation policy at start, queued admission, conflict resolution and completion revalidation, preserving existing mutation consumers, owner/Origin/scope and project containment. Download options accept no public target or private-path/descriptor selectors; an optional safe basename is display metadata only. Registered internal artifact capability grants no project write and never forges a writable scope. Artifact GET refreshes read authority under shared R39 admission, with no MCP/session global grant — kind-static readOnly and writable-only conflict resolve would either reject headless folder downloads or admit unauthorized file-output mutations — cost if wrong: admission, consent and checked source opening require rework, and read-only download support may be unavailable until corrected.

Ruling: T15 separate bounded ZIP output — Charge actual admitted source payload once across all roots against jobBytes and describe that payload in public progress. Separately enforce a private finite output allowance for fixed yazl3.3.1 compression/options with no comments: 2 * plannedPayload + sum(2 * UTF8(memberName).length + 256) + 256, including every written directory member and trailing slash. Check safe integer arithmetic before staging and each output chunk before owned writes; enforce captured input size/revision, aggregate actual bytes, final source validation and awaited bounded reports. The upload-only 10-GiB cap does not apply to archive source files; sync/proof reads do not inflate payload progress. Root checked the pinned writer's at-most137 fixed member bytes plus two name copies,98 ZIP64 footer bytes and zlib's at-most13.5-percent-plus-eleven-byte expansion; the combined allowance is a conservative implementation inference, not a universal ZIP formula. Revisit it if writer fields, comments or compression/flush settings change. No new runtime tool, public budget or config knob is needed — counting input and output together halves usable payload, while the same output cap rejects legal at-limit incompressible data due to ZIP overhead — cost if wrong: byte enforcement/progress need rework, and the conservative private allowance permits additional scratch storage beyond source payload.

Ruling: T15 journaled archive artifacts — Extend the existing publisher/completion journal with a distinct archive association bound to actual job/scope, immutable mode/target, manifest version and omission consent before stage creation; finished ZIP evidence is recorded after streaming/sync and compared with owned payload proof before readiness. Public-file output uses ordinary conflict/no-replace/exchange/trash adoption; download output uses only a server-derived registered private location and internal capability with existing parent/inode/creation-intent proof and finite cleanup. Atomically checkpoint proven output, bounded results/counters and completion marker before cleanup; unresolved adoption/cleanup is not full success. Completed request replay never recreates or republishes; startup reconciles proof/interruption without restarting source work. Retain explicit private-artifact disposition until durable cleanup, with recovery that does not resolve it as a public path. Only successful terminal jobs with current proven completed regular artifacts may download; changed, missing, uncertain and partial bytes remain unavailable. Scope the actual job, share R39 admission, revalidate inside it, preserve owned opens and safe attachment headers; unfinished GET is409 and unknown/expired follows scoped not-found. Extend existing seven-day paged sweep/prune: exclude accepted queued/active consumers through private claims in the same lifecycle owner, delete only proved registered private bytes, record disposition, then prune metadata; drain consumers/sweep before store close. Uncertainty stays pinned, and published user ZIPs are never retention-deletion targets. Task17 retains artifactReady projection/presentation — storageRoot protection and copy/move completion associations cannot represent retained download artifacts and their pruning obligations without an explicit internal journal contract — cost if wrong: capability/recovery/retention need rework, and uncertain private data can consume storage until disposition is proven.

Ruling: T18 durable text-save result — Extend the existing shared request/job/publication association and completion dispatcher with a distinct internal text_save kind using existing FileJobs admission/workers/lifecycle, not another journal, request namespace, queue or fake copy/upload. Bind the actual scoped request, selected pathname, explicit create-or-replace precondition, expected d1 or F5 creation marker, accepted raw byte length and SHA256 before staging; persist no raw contents in metadata. Existing publication fields bind canonical target/link and stage identity, and completed stage bytes must match the body digest. One active request owns one writer; matching duplicates join/observe, changed body/path/precondition rejects, and queued writes allocate no stage. Capture exact path/d1/metadataRevision-e1 from one accepted published observation, atomically checkpoint proof/result before cleanup, and finalize success only after required displaced adoption/cleanup. Completed identical retry returns the immutable recorded pair even after a later target edit, without another save or substituting newer metadata. Startup reuses existing proof/completion/finalization without receiving/saving again; generic recovery e1 is not d1. Retain any committed pair; if absent, derive a consistent pair outside leases only from still-proven publication, without claiming an unrecorded historical generation. Result/disposition markers survive proven container cleanup and pruning. Uncertain evidence stays unresolved; accepted work drains through disconnect/shutdown, and cleanup remains limited to registered proven unpublished work. Generic operation requests cannot forge this internal kind; F1 includes later kind/catalog/projection integration — current request storage and transfer/upload completion do not bind text body/preconditions or durable paired d1/e1 results, while recovery supplies e1 alone — cost if wrong: request/publication/recovery integration needs rework, and an uncertain save must preserve its draft and obligations instead of being repeated.

Ruling: T18 document metadata observation — Add GET /metadata?path=<selectedPath>&view=document under both existing prefixes, returning only scoped public path, resolvedPath and metadataRevision. The e1 domain is the followed regular target stat plus full selected-link identity, matching the observation paired with FileDocument/save d1. Omitted view or view=entry retains ordinary non-follow FileEntry semantics; unknown/duplicate/malformed views reject. The explicit document read uses current scope and owned descriptors, revalidates target/link and reads no content or hash, exposing no private capability or p1. Readonly/headless readable documents remain observable with existing owner/GET protections and no new token grant. Add metadataRevision to the same accepted shared d1 observation, not a later independent lookup or another revision formula. Tasks19/20 poll this variant for the retained selected path and compare e1 only with e1, detecting target edits and same-target link replacement; d1 remains the save precondition — ordinary metadata describes the non-follow leaf link, so its revision either falsely differs from the document or misses target changes, while resolved-target-only polling misses selected-link changes — cost if wrong: the small endpoint/helper/editor integration needs rework, with false stale notices or missed changes until corrected.

R40 implementation clarification: Pin both NFD passes to Unicode15.1 canonical decomposition and combining-class data, including algorithmic Hangul and stable canonical ordering, alongside the pinned C/F fold. Runtime ICU must not change the persisted policy for characters unassigned in15.1. Ship the official data and license through installation/update and verify the bounded helper, provenance and distribution. This completes the deterministic product rejection rule without a native ABI or actual-filesystem equivalence claim. See the [Unicode15.1 normalization annex](https://www.unicode.org/reports/tr15/tr15-54.html).

Ruling: T14 selected upload current-child relation — Add GET /jobs/:groupId/upload-children?cursor under both existing Explorer prefixes, returning {children:[{entryId,job:FileJob|null}],nextCursor}. Validate the trusted scoped upload_group and its journal, page only its current manifest associations by sequence with at most200 rows and the effective manifest cap, and bind opaque cursors to scope and group. Join each association only to a child matching scope, parent job, authoritative entry ID and file/upload or directory/create_directory type; expose only the existing public FileJob, never raw currentJobId, operation, inode, hash or private recovery data. Missing or mismatching child metadata returns null without disclosing another scope or implying completion, cancellation or retry eligibility; rows without associations are omitted. Use the existing serial FileJobClient/useFileJobs observer and shared page budget for selected-group discovery, refreshing from explicit page one and evicting unseen mappings only after a complete successful bounded live sweep. Do not scan historical attempts or permanently poll every completed child. Automatically present the selected group's actual directory conflicts, including beyond general history page200, and bind recovered cancel/resolve gestures to the returned child/conflict IDs without retargeting stale actions or sending bytes. Existing mutation/auth/scope guards and group cancellation remain authoritative. Implement the small store/delegate/read-route and browser integrations in Task14; Task13's completed receiver contracts remain unchanged — the current public manifest omits private child IDs and general history cannot reliably expose a selected group's pending directory conflict or recovered file control — cost if wrong: current-child projection, live pagination and UI ownership need rework; unavailable child details remain explicit, and live pages do not promise an atomic historical snapshot.
