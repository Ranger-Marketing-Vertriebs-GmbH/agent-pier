...
export function trustedPeers(h,ps,peerHome=h){
 return listPeers(peerHome,ps).filter(peer=>{
...
export function registerPeer(ctx,nativeSessionId,{pid,ps=pidStart}={}){
 ...
 const {launch,h}=ctx;const peerHome=ctx.peerHome||h;
 ...
 const bound=trustedPeers(h,ps,peerHome).find(peer=>peer.agentpierSessionId===launch.id&&peer.alive);
 ...
 register(peerHome,peer);return {...peer,key};
}
...
export async function trustedNudge(h,peer,text,from,deps={}){
 const current=trustedPeers(h,deps.ps,deps.peerHome||h).find(item=>item.key===peer.key&&item.alive);if(!current)return false;
