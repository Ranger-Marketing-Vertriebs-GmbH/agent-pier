export const pipelineCopy = {
  ...
  runStageProgress: (done, total) => `${done} of ${total} stages completed`,
  runProgressShort: (done, total, stage) =>
    stage ? `${done}/${total} · ${stage}` : `${done}/${total}`,
  runStarted: (time) => `Started: ${time}`,
  progress: "Progress",
  updated: "Updated",
  allRuns: "All runs",
  runBranch: (branch) => `branch ${branch}`,
  runStartedMeta: (time) => `started ${time}`,
  stageFlags: {
    gate: "Approval",
    verify: "Verification",
    pr: "Pull request",
  },
  loopTo: (stage) => `↺ to stage ${stage}`,
  decisionCue: (stage) => `Decision required · stage ${stage}`,
  attemptNumber: (number) => `Attempt ${number}`,
  severities: { low: "Low", medium: "Medium", high: "High" },
  stepLog: (name) => `Verification log: ${name}`,
  ...
  editStage: (number) => `Edit stage ${number}`,
  moveForward: "Move forward",
  moveBackward: "Move back",
  stageCount: (count) => (count === 1 ? "1 stage" : `${count} stages`),
  pipelineCount: (count) => (count === 1 ? "1 pipeline" : `${count} pipelines`),
  noStageFlags: "No extra steps",
  loopCaption: (name, stage, rounds) =>
    `${name} returns to stage ${stage} on failure · max. ${rounds} rounds`,
  decreaseRounds: "Decrease loop rounds",
  increaseRounds: "Increase loop rounds",
